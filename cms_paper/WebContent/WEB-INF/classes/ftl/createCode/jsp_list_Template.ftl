				...
				<tr>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="field1" value="" placeholder="这里输入关键字" />
					...
					<td style="vertical-align:top;"><button class="btn btn-mini btn-light" onclick="search();"  title="搜索"><i id="nav-search-icon" class="icon-search"></i></button></td>
				</tr>
			</table>
					...
					<c:when test="${r"${not empty varList}"}">
						<c:forEach items="${r"${varList}"}" var="var" varStatus="vs">
							<tr>
								...
								<td  class="center">
									 <a class='btn btn-mini btn-info' title="编辑" onclick="edit('${r"${var."}${objectNameUpper}_ID${r"}"}');"><i class='icon-edit'></i></a>
				 					 <a class='btn btn-mini btn-danger' title="删除" onclick="del('${r"${var."}${objectNameUpper}_ID${r"}"}');"><i class='icon-trash'></i></a>
								</td>
							</tr>
