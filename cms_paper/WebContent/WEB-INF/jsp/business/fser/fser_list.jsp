...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="fser/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								
								<td style="vertical-align:top;padding-left:2px;">
								 	<select class="chosen-select form-control" name="USER_STATE" id="user_state" data-placeholder="状态筛选" style="vertical-align:top;width: 120px;" >
									<option></option>
								...
								<td>
									...
									<span class="input-icon">
										<input class="nav-search-input" autocomplete="off" id="nav-search-input" type="text" name="NICK_NAME" placeholder="输入用户名关键词" />
								...
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'><a onclick="viewUser('${var.USER_ID}')" style="cursor:pointer;">${var.NICK_NAME}</a></td>
											<td class='center'>${var.EMAIL}</td>
											<td class='center'>${var.REGISTER_TIME}</td>
											<td class='center'><c:if test="${var.USER_STATE == 'N'}" >已停用</c:if><c:if test="${var.USER_STATE == 'Y'}" >使用中</c:if></td>
											<td class='center'>${var.LAST_LOGIN_TIME}</td>
											<td class="center">
												<div class="hidden-sm hidden-xs btn-group">
													<a class="btn btn-xs btn-warning" title="用户状态锁" onclick="lock('${var.USER_ID}','${var.USER_STATE}');">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>fser/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function viewUser(Id){
			if('admin' == Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>fser/view.do?USER_ID='+Id;
			 diag.Width = 469;
			 diag.Height = 380;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>fser/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
