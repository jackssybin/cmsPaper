						...
						<table id="table_report" class="table table-striped table-bordered table-hover">
							<tr>
								...
								<td><input type="text" name="channelId" id="channelId" value="${article.channelId}" />
									<input class="treeText" readonly="readonly" type="text"  id="channelName" name="channelName" style="width: 65%;" onFocus="showMenu('channelTreeContent',this)" />
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td><textarea rows="3" cols="20"  name="shortContent" id="shortContent" value="${article.shortContent}" placeholder="这里输入摘要" title="摘要" style="width:98%;">${article.shortContent}</textarea></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td><input type="hidden" name="pic" id="pic" value="${article.pic}" maxlength="200" placeholder="这里输入缩略图" title="缩略图" style="width:98%;"/>
									<div class="col-xs-12">
									<label class="pos-rel col-xs-2"><input type="checkbox" class="ace" id="getImgFromContent"  /><span class="lbl">提取缩略图</span></label>
									<div class="col-xs-10" id="allExtraPics">
										缩略图
									</div>
							...
							<tr>
								...
								<td><input type="hidden" name="weight" id="weight" value="${article.weight}" style="width:50%;"/>
									<div class="rating inline"></div><span id="weightShow"></span>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<td style="width:175px;text-align: right;padding-top: 13px;">置顶类型:</td>
								<td><input type="hidden" name="topType" id="topType" value="${article.topType}" maxlength="200" placeholder="这里输入置顶类型" title="置顶类型" style="width:98%;"/>
									<label style="float:left;padding-left: 8px;padding-top:7px;" >
										<input name="form-field-radio4" type="radio" class="ace"  <c:if test="${article.topType == '1' }">checked="checked"</c:if> onclick="setType('topType','1');"/>
										<span class="lbl"> 无</span>
									</label>
									<label style="float:left;padding-left: 5px;padding-top:7px;">
										<input name="form-field-radio4" type="radio" class="ace" <c:if test="${article.topType == '2' }">checked="checked"</c:if> onclick="setType('topType','3');"/>
										<span class="lbl"> 栏目</span>
									</label>
									<label style="float:left;padding-left: 5px;padding-top:7px;">
										<input name="form-field-radio4" type="radio" class="ace" <c:if test="${article.topType == '3' }">checked="checked"</c:if> onclick="setType('topType','3');"/>
										<span class="lbl"> 首页</span>
									</label>
								</td>
							</tr>													
							<tr>
...
<!-- /.main-container -->
	...
	<script type="text/javascript" src="static/ace/js/jquery.raty.min.js"></script>
    ...
    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
		...
		$(document).ready(function(){
			...
			//栏目树			
		  createSelectSimpleZtree("channelTree", "channelTreeContent","channel/commonTree.do", "channelName","channelId", "id=pId", channelSeting);
			//权重栏
			$('.rating').raty({
					'cancel' : true,
				    'cancelHint':   '取消权重!',
			...
			//缩略图
			 $("#getImgFromContent").click(function(){ 
      			  if($("#getImgFromContent").prop("checked")==true){ 
           			 alert('1231');//do some thing
           			 var img='<img src="plugins/ueditor/jsp/upload/image/20160704/1467622781507046719.png" title="1467622781507046719.png" alt="1.png"/>"' ;
           			 $("#allExtraPics").append(img);
           			 getImgFromHtml();
		...
		function getImgFromHtml(){
			...
			var imgReg = "/<\s*img\s+[^>]*?src\s*=\s*(\'|\")(.*?)\1[^>]*?\/?\s*>/";
			//var srcReg = '/src=['"]?([^'"]*)['"]?/i';
			alert(allHtml)
			var arr = allHtml.match(imgReg);
			...
			for (var i = 0; i < arr.length; i++) {
				...
				//alert('已匹配的图片地址'+(i+1)+'：'+arr[i]);
				 $("#allExtraPics").append(arr[1]);
				//}
				}
			return imgArrs;
		}
