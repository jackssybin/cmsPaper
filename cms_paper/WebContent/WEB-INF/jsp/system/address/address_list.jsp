						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">联系人所在市</th>
									<th class="center">联系人所在区</th>
									<th class="center">所在区详细</th>
									<th class="center">详细地址</th>
									<th class="center">邮箱</th>
									<th class="center">别名</th>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.ADDRESS_ID}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center'>${var.USER_ID}</td>
											<td class='center'>${var.LINKER}</td>
											<td class='center'>${var.MOBILE}</td>
											<td class='center'>${var.PHONE}</td>
											<td class='center'>${var.AREA}</td>
											<td class='center'>${var.ZONE}</td>
											<td class='center'>${var.ZONE_DETAIL}</td>
											<td class='center'>${var.DETAIL_ADDRESS}</td>
											<td class='center'>${var.EMAIL}</td>
											<td class='center'>${var.ALIAS}</td>
											<td class='center'>${var.IS_DEFAULT}</td>
											<td class="center">
												<div class="hidden-sm hidden-xs btn-group">
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.ADDRESS_ID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													<a class="btn btn-xs btn-danger" onclick="del('${var.ADDRESS_ID}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.ADDRESS_ID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<li>
																<a style="cursor:pointer;" onclick="del('${var.ADDRESS_ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>address/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
